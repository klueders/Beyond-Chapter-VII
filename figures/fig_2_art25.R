...
df_tmp <- df %>%
  group_by(decade) %>%
  summarise(art25 = sum(art25_charter))

p <- ggplot(df_tmp, aes(x=factor(decade), y=art25)) +
  geom_col() +
  scale_x_discrete(labels = decades) +
  scale_y_continuous(limits = c(0, 10), breaks = seq(0, 10, 2)) +
  labs(title = "References to Article 25 over time", y = "Number of Resolutions", x = NULL) +
  theme_bw() +
  theme_bw_academic
p

ggsave("fig_2_art25.png",p, width = 8.5, height = 5, dpi=600)
